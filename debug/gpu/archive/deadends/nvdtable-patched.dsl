 ...
 */
DefinitionBlock ("", "SSDT", 1, "Insyde", "NvdTable", 0x00001003)
{
            ...
            Switch (ToInteger (Arg2))
            {
                ...
                Case (0x2A)
                {
                    ...
                    Switch (ToInteger (PSH0))
                    {
                        ...
                        {
                            RETN = 0x0102
                            // ПАТЧ v3: целевая температура GPU всегда 0x53 (83 °C),
                            // как в производительном режиме (STNM==3, PDFG&8).
                            // Было: 0x42 (66 °C) при STNM!=3 — единственное
                            // оставшееся STNM-зависимое значение в таблице,
                            // главный кандидат на источник лока 17 Вт.
                            TGPU = 0x53
                            Return (GPSP) /* \_SB_.PC00.PEG0.PEGP.GPSP */
                        }
    ...
    Scope (\_SB)
    {
        Device (NPCF)
        {
            ...
            Method (_DSM, 4, Serialized)  // _DSM: Device-Specific Method
            {
                ...
                {
                    If ((\_SB.PC00.PEG0.PEGP.NPCS != Zero)) // ПАТЧ v2: гейт восстановлен — канал должен отвечать
                    {
            ...
            Method (NPCF, 4, Serialized)
            {
                ...
                Switch (ToInteger (Arg2))
                {
                    ...
                    Case (0x02)
                    {
                        ...
                        {
                            TGPA = Zero // ПАТЧ v2: CTGP-оффсет всегда 0 (было ACBT — EC мог двигать)
                            TGPD = DCBT /* \_SB_.NPCF.DCBT */
                            PC01 = Zero
                            PC02 = Zero // ПАТЧ v2: буст-контроллер ВСЕГДА выключен (было DBAC|DBDC<<1 — источник лока)
                            TPPA = 0x0168 // ПАТЧ v2: TPP 45 Вт вместо кривых 15/20 (было ATPP)
